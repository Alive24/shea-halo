You are working on Shea Halo issue {{ issue.identifier }}.
...
- `README.md`
- `pyproject.toml`
- `examples/halo-research-issue.md`
...
6. Implement only the accepted issue scope. Preserve Shea Halo's Issue/Project product surface, append-only workpad, managed workspace, experimental-snapshot-only branch, framework-native observation, HALO Engine, and credential-redaction boundaries. Add concise comments at non-obvious runtime, schema, retry/idempotency, compatibility, or external-service boundaries; do not add comments that merely restate assignments or function names. When changing exported Python APIs, Pydantic models, configuration fields, or public runtime behavior, audit whether the surface should remain public and add documentation where future consumers need its contract.
7. Run the verification required by the issue and the workflow's `verification.commands`: `uv sync --locked --all-groups`, `uv run ruff format --check .`, `uv run ruff check .`, `uv run basedpyright`, `uv run pytest`, and `uv build`. Repair in-scope failures, then rerun the relevant checks. Use package-scoped or additional commands only when the changed surface requires them, and never claim verification that was not actually run.
...
- API/schema documentation and public-visibility audit results for new or changed exported Python, Pydantic, or configuration surfaces, or `not applicable`.
